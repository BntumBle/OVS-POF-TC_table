...
typedef void xlate_actions_handler(const struct ofpact *, size_t ofpacts_len,
                                   struct xlate_ctx *, bool, bool,struct pof_flow *,
                                   struct pof_fp_flow_wildcards *, int act_num);
...
static void xlate_table_action_pof(struct xlate_ctx *, ofp_port_t in_port,
                               uint8_t table_id, bool may_packet_in,
                               bool honor_table_miss, bool with_ct_orig,
                               bool is_last_action, xlate_actions_handler *,int action_num);
...
xlate_recursively(struct xlate_ctx *ctx, struct rule_dpif *rule,
                  ...
                  xlate_actions_handler *actions_xlator, int action_num)
{
    ...
    /*action_num++;*//*zq:not sure*/
    VLOG_INFO("+++++++zq xlate_recursively action_num:%d\n", action_num);
    actions_xlator(actions->ofpacts, actions->ofpacts_len, ctx,
                   is_last_action, false, flow,wc, action_num);
...
static void
xlate_table_action_pof(struct xlate_ctx *ctx, ofp_port_t in_port, uint8_t table_id,
                   bool may_packet_in, bool honor_table_miss,
                   bool with_ct_orig, bool is_last_action,
                   xlate_actions_handler *xlator, int action_num)
{
    /* Check if we need to recirculate before matching in a table. */
    if (ctx->was_mpls) {
        ctx_trigger_freeze(ctx);
    ...
    if (xlate_resubmit_resource_check(ctx)) {
        ...
        if (rule) {
            ...
            xlate_report_table(ctx, rule, table_id);
            xlate_recursively(ctx, rule, table_id <= old_table_id,
                              is_last_action, xlator, action_num);
            ctx->xin->trace = old_trace;
...
pof_do_xlate_actions__(const struct ofpact *ofpacts, size_t ofpacts_len,
                     struct xlate_ctx *ctx, bool is_last_action,
                     bool group_bucket_action, struct pof_flow *flow,
                             struct pof_fp_flow_wildcards *wc, int action_num)
{
    const struct ofpact *a;
...
//            compose_mpls_push_action(ctx, ofpact_get_PUSH_MPLS(a));
            ...
            case OFPACT_GOTO_TABLE: {
                ...
                xlate_table_action_pof(ctx, ctx->xin->flow.in_port.ofp_port,
                                   ogt->table_id, true, true, false, last,
                                   pof_do_xlate_actions__, action_num);
                break;
            }
                 ...
                 bool group_bucket_action)
{
    ...
    pof_do_xlate_actions__(ofpacts, ofpacts_len, ctx, is_last_action, group_bucket_action, flow, wc, action_num);
}
